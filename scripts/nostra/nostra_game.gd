...
func update_score_labels(player_score: int, npc_score: int):
	$Player_Info/VBoxContainer/Player_Score.text = str(player_score) + "/" + str(needed_scores.player_needed_score)
	$Enemy_Info/VBoxContainer/Enemy_Score.text = str(npc_score) + "/" + str(needed_scores.npc_needed_score)
...
func add_cards_to_discard(pairs: Array[Dictionary]):
	...
	update_score_labels(scores.player_score, scores.npc_score)
...
func _on_button_show_end_result_pressed():
	var player_age_total = score_handler.sum_card_age(player_discard_pile)
	var npc_age_total = score_handler.sum_card_age(npc_discard_pile)
